...
void IMUupdate(float gx, float gy, float gz, float ax, float ay, float az)
{
  ...
  // estimated direction of gravity and flux (v and w)              重力加速度和地磁场方向/数值
  vx = 2*(q1q3 - q0q2);                                             //四元数与xyz的表示
  vy = 2*(q0q1 + q2q3);
  vz = q0q0 - q1q1 - q2q2 + q3q3 ;
 
  // error is sum of cross product between reference direction of fields and direction measured by sensors
  ex = (ay*vz - az*vy) ;                                             //用外积误差来修正陀螺仪的漂移误差
  ey = (az*vx - ax*vz) ;
  ez = (ax*vy - ay*vx) ;
 
  exInt = exInt + ex * Ki * 2 * halfT;                                //误差积分
  eyInt = eyInt + ey * Ki * 2 * halfT;
  ezInt = ezInt + ez * Ki * 2 * halfT;
 
  // adjusted gyroscope measurements
  gx = gx + Kp*ex + exInt;                                              //通过PI补偿陀螺仪角速度，消除陀螺仪漂移
  gy = gy + Kp*ey + eyInt;
  gz = gz + Kp*ez + ezInt;                                          //这里gz没有观测矢量进行融合校正，可能出现累计误差，需要人工零偏校正
